...
def model(img):
    ...
    cx = int(center['m10'] )/int( center['m00']) #centroid
    cy = int(center['m01']) /int( center['m00'])
    cv2.circle(img2, (int(cx), int(cy)), int(r), (255, 255, 255), -1)
    ...
    for i in circles[0,:]:
        cv2.circle(cimg_morpho,(int(i[0]),int(i[1])),int(i[2]),(0,255,0),2)
        cv2.circle(cimg_morpho,(int(i[0]),int(i[1])),2,(0,0,255),3)
